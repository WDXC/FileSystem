...
void log_conn(struct fuse_conn_info* conn) {
    ...
    // kernel congestion threashod parameter
...
void log_fi(struct fuse_file_info* fi) {
    ...
    // can be filled by open , to use direct I/O on this file
    // Introduced in version 2.4
    // unsigned int keep_cache : 1
    log_struct(fi, direct_io, %d, );

    // can be filled in by open, to indicate, that cached file data
    // need not be invalidated. Introduced in version 2.3
...
void log_statvfs(struct statvfs* sv) {
    ...
    // unsigned logn f_bsize; /* file system block size*/
